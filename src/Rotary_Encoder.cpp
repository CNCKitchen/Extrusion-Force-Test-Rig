...
Rotary_Encoder::Rotary_Encoder(const uint8_t dataPin):_dataPin(dataPin)
{}


// PCNT interrupt handler: called when the hardware counter hits the high limit.
void Rotary_Encoder::IRAM_ATTR pcnt_intr_handler(void *arg) {
  uint32_t status = 0;
...
uint32_t Rotary_Encoder::calc_length(uint32_t totalPulses) {
  uint32_t lenght_mm = totalPulses / Pulses_per_mm;
  return lenght_mm;
}

uint32_t Rotary_Encoder::get_length (){
    int16_t count = 0;
